...
class ShipOrder:
    """卖家发货操作"""
    def __init__(self, seller, order_id):
...
class QueryOrders:
    ...
    def run(self) -> (bool, int):
        ...
        return code == 200, len(orders) if orders else 0

class Search:
    def __init__(self , buyer: Buyer ,keyword,field,store_id,page,page_size):
        self.buyer = buyer
    ...
    def run(self) -> (bool, int):
        code,results = self.buyer.search(self.keyword,self.field,self.store_id,self.page,self.page_size)
        if code != 200:
            return False, 0
            # results:
            # {
...
class Workload:
    def __init__(self):
        ...
        self.n_ship_order_past = 0
        self.n_receive_order_past = 0
        self.n_cancel_order_past = 0
        self.n_query_orders_past = 0
        self.n_new_order_ok_past = 0
        self.n_payment_ok_past = 0
        self.n_ship_order_ok_past = 0
        self.n_receive_order_ok_past = 0
        self.n_cancel_order_ok_past = 0
        self.n_query_orders_ok_past = 0
        # 存储卖家对象和店铺映射，用于发货操作
        self.sellers = {}  # {store_id: seller}
        self.store_to_seller_id = {}  # {store_id: seller_id}
    ...
    def gen_database(self):
        ...
        from fe.access.seller import Seller as SellerAccess
        for i in range(1, self.seller_num + 1):
            ...
            # 保存seller访问对象，用于发货操作
            seller_access = SellerAccess(url_prefix=conf.URL, seller_id=user_id, password=password)
            for j in range(1, self.store_num_per_user + 1):
                ...
                # 保存店铺到卖家的映射
                self.sellers[store_id] = seller_access
                self.store_to_seller_id[store_id] = user_id
                row_no = 0
    ...
    def get_new_order(self) -> NewOrder:
        ...
        # robust inclusive index selection
        ...
        for i in range(0, books):
            book_no = random.randint(0, len(self.book_ids[store_id]) - 1)
    ...
    def update_stat(
        ...
        self.n_ship_order = self.n_ship_order + n_ship_order
        self.n_receive_order = self.n_receive_order + n_receive_order
        self.n_cancel_order = self.n_cancel_order + n_cancel_order
        self.n_query_orders = self.n_query_orders + n_query_orders
        self.n_new_order_ok = self.n_new_order_ok + n_new_order_ok
        self.n_payment_ok = self.n_payment_ok + n_payment_ok
        self.n_ship_order_ok = self.n_ship_order_ok + n_ship_order_ok
        self.n_receive_order_ok = self.n_receive_order_ok + n_receive_order_ok
        self.n_cancel_order_ok = self.n_cancel_order_ok + n_cancel_order_ok
        self.n_query_orders_ok = self.n_query_orders_ok + n_query_orders_ok
        self.time_new_order = self.time_new_order + time_new_order
        self.time_payment = self.time_payment + time_payment
        self.time_ship_order = self.time_ship_order + time_ship_order
        self.time_receive_order = self.time_receive_order + time_receive_order
        self.time_cancel_order = self.time_cancel_order + time_cancel_order
        self.time_query_orders = self.time_query_orders + time_query_orders
        # 计算这段时间内新创建订单的总数目
        n_new_order_diff = self.n_new_order - self.n_new_order_past
        # 计算这段时间内新付款订单的总数目
        n_payment_diff = self.n_payment - self.n_payment_past

        # 防止分母为0：需要有新增的订单和付款记录，且耗时为正
        if (
            self.n_payment != 0
            and self.n_new_order != 0
            and n_new_order_diff > 0
            and n_payment_diff > 0
            and (self.time_payment + self.time_new_order) > 0
        ):
            ...
            # 输出新功能的统计信息
            logging.info(
                "Ship=OK:{} Total:{} Latency:{} | Receive=OK:{} Total:{} Latency:{} | Cancel=OK:{} Total:{} Latency:{} | Query=OK:{} Total:{} Latency:{}".format(
                    self.n_ship_order_ok,
                    self.n_ship_order,
                    self.time_ship_order / self.n_ship_order if self.n_ship_order > 0 else 0,
                    self.n_receive_order_ok,
                    self.n_receive_order,
                    self.time_receive_order / self.n_receive_order if self.n_receive_order > 0 else 0,
                    self.n_cancel_order_ok,
                    self.n_cancel_order,
                    self.time_cancel_order / self.n_cancel_order if self.n_cancel_order > 0 else 0,
                    self.n_query_orders_ok,
                    self.n_query_orders,
                    self.time_query_orders / self.n_query_orders if self.n_query_orders > 0 else 0,
                )
            )
        self.lock.release()
        # 旧值更新为新值，便于下一轮计算
    ...
    def get_search(self) -> Search:
        ...
        possible_keywords = ["沙漠", "爱情", "科幻", "战争", "历史", "梦"]
        keyword = random.choice(possible_keywords)

        field = random.choice(["title", "content", "book_intro", "tags", "all"])

        store_id_in = random.choice(self.store_ids) if self.store_ids and random.random() < 0.5 else None

        page = random.randint(1, 3)
        page_size = random.choice([5, 10, 20])

        return Search(b, keyword, field, store_id_in, page, page_size)